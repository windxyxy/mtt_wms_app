...
public class OrderParse {
    ...
    void ProData(){
        ...
        Log.d(OrderParse.class.getName(),"退出消息循环处理线程");
    }
    ...
    void InvokeMethod(TranCoreClass tcc){
        for(PostClass pc : tcc.PostClassT){
            ...
            Log.d(OrderParse.class.getName(),"处理指令:" + String.valueOf(pc.getInjType())+";指令体:"+pc.getCoreObject());

            IOrderTran ot = OrderRegister.GetInstance().Get(String.valueOf(pc.getInjType()));
            if(ot!=null)
                ot.Tran(tcc);
            else
                Log.d(OrderParse.class.getName(), "指令" + String.valueOf(pc.getInjType()) + " 没有找到对应的方法:");
